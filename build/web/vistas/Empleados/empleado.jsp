    ...
    <body>
       ...
       <hr />
         <table cellspacing="0"  id="permisos" class="table table-bordered" > 
          ...
          <tbody>
        <%ArrayList<Empleado> std = (ArrayList<Empleado>)request.getAttribute("data"); 
        for(Empleado s:std){%> 
            <tr> 
                <td><%=s.getDni()%></td> 
                <td><%=s.getNombreEmpleado()  %></td> 
                <td><%=s.getApellidoEmpleado()   %></td> 
                <td><%=s.getDireccionEmpleado() %></td> 
                <td> 
        <form method='post' id='emp-AddForm' action='EAlumno'>
            <input type="hidden" name="ida" value="<%=s.getDni()%>">
        ...
        <form method='post' id='emp-AddForm' action='RAlumno'>
            <input type="hidden" name="dni" value="<%=s.getDni()%>">
